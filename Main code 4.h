...
// Hàm hiệu chỉnh hướng (rẽ trái hoặc phải) 
void correctHeading(double targetAngle) {
    // Đọc góc hiện tại từ cảm biến Inertial
    double currentAngle = Inertial11.heading(); 

    // Tính độ lệch giữa góc hiện tại và mục tiêu
    double error = targetAngle - currentAngle;
    ...
    while (fabs(error) > 1.0) {  // Dừng khi sai số nhỏ hơn 1 độ
        ...
        // Tính tốc độ điều chỉnh (sử dụng hệ số để giảm sai số)
        double speed = error * 0.5;

         // Nếu độ sai lệch nhỏ (gần mục tiêu), giảm tốc độ
        if (fabs(error) < 10.0) {
            speed = speed * 0.5;  // Giảm tốc độ để tránh quay quá nhanh
        }

        if (error > 0) {
            // Quay phải: Motor trái quay về phía trước, motor phải quay ngược
            ...
            // Quay trái: Motor trái quay ngược, motor phải quay về phía trước
        ...
        // Chờ để ổn định trước khi đo lại góc
        vex::task::sleep(20);
    }

    // Dừng động cơ sau khi đạt được góc cần thiết
    leftMotor.stop();
    rightMotor.stop();
}
